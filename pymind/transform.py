...
class TransformInterface:
    ...
    def multiscale(self, img, th, maxIt=500, sigma=None, tau=None, theta=1, toDisp=2, check=50,
        ...
        while it < maxIt:
            ...
            y = prox.proxl1(y, sigma * th)
            self.ctranspose()
            x = xo - tau * numpy.real(self.forward(y))
            self.ctranspose()

            if rType == 'TV':
                x = prox.proxTV(x, tau, x, nitTV, tolTV / it, (it % check == 0) and (toDisp > 0))
            elif rType == 'HK':
                x = prox.proxHk(x, tau, k_sob, (it % check == 0) and (toDisp > 0))
            elif rType == 'chTV':
                x = prox.chTV(x, tau, nitTV, tolTV / it, (it % check == 0) and (toDisp > 0))
            ...
            if rType == 'TV' or rType == 'ch':
                dx1, dx2 = prox.num_grad(x)
                stat['oVal'][it - 1] = numpy.sum(numpy.sqrt(dx1[:]**2) + dx2[:]**2) / img.size
            elif rType == 'HK':
                stat['oVal'][it - 1] = 0.5 * prox.l2norm_der_ft(x, k_sob)
